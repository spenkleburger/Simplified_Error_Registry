# Simplified Error Registry: Architecture & Implementation Plan
...
- **fix_repo.md**: Solutions to **errors in application and test code**. When code fails at runtime or tests fail, this file stores which code change resolved it. Use it **reactively** when you have a concrete error to fix.
- **coding_tips.md**: **Agent process and workflow rules**. How the agent should behave (paths, commands, tooling, conventions) to avoid causing errors or workflow failures. Use it **proactively**; it is built only from **Agent Process Issue** entries.
...
│    - Agent checks lookup order:                             │
│      a. .errors_fixes/errors_and_fixes.md (recent session)  │
│      b. .errors_fixes/fix_repo.md (consolidated fixes)     │
│      c. .errors_fixes/coding_tips.md (process rules)      │
...
│    - Agent checks lookup order (same as manual mode)       │
...
### Approach 2: Automatic Test Execution (Not Recommended)

**Workflow**: Agent writes code → Agent runs tests automatically → Agent fixes errors → Repeats until all pass

#### Why Not Recommended
- ❌ **High Token Usage**: Each test run consumes tokens
- ❌ **Safety Risk**: Agent might run destructive tests
- ❌ **Less User Control**: Tests run without explicit approval
- ❌ **Can Be Slow**: Long test suites delay feedback
- ❌ **Infinite Loop Risk**: Agent might keep trying same fix
- ❌ **Resource Intensive**: Runs tests even when code is clearly broken
- ❌ **No User Review**: Code changes without human oversight

**Note**: This approach is intentionally excluded from the Simplified Error Registry design for safety and efficiency reasons.
...
1. **Determine Test Command**
   - Check what code was modified (backend, frontend, integration)
   - Use appropriate test command:
     - Backend changes → `task test:backend` or `task test` (if backend-only)
     - Frontend changes → `task test:frontend`
     - Integration changes → `task test:integration`
     - Mixed changes → Run all relevant test suites sequentially

2. **Run Tests**
   - Execute determined test command(s)
   - Capture full test output (stdout and stderr)
...
3. **Check Results**
   ...
   - If tests fail:
     ...
     - Extract error signatures, types, and context
     - Proceed to fix loop

4. **Fix Loop** (Maximum 5 iterations)
   - For each error found:
     - Check `.errors_fixes/errors_and_fixes.md` for recent session errors
     - Check `.errors_fixes/fix_repo.md` for consolidated fixes (try fixes in order of success count)
   ...
   - If all tests pass: Stop and report success
   - If same errors persist after 3 attempts: Stop and report to user
   - If new errors appear: Continue fixing
   - If iteration count reaches 5: Stop and report status

5. **Final Report**
   - Summarize all fixes applied
   - List any remaining errors
   - Update `.errors_fixes/errors_and_fixes.md` with final status

## Safety Limits

- **Maximum Iterations**: 5 fix cycles
- **Same Error Retry Limit**: 3 attempts per error
- **Stop Conditions**:
  - All tests pass
...
Always document in `.errors_fixes/errors_and_fixes.md`:
- Each error encountered
- Each fix attempted
- Result (✅ Solved / ❌ Failed)
- Success count (if fix already exists)
- Test command used (backend/frontend/integration)
```
...
└─────────────────────────────────────────────────────────────┘
                    ↓
┌─────────────────────────────────────────────────────────────┐
│ 3. User runs `task test`                                    │
└─────────────────────────────────────────────────────────────┘
                    ↓
┌─────────────────────────────────────────────────────────────┐
│ 4. If tests fail:                                           │
│    - User provides error output to agent                   │
│    - Agent checks .errors_fixes/ files                      │
│    - Agent applies fix                                      │
│    - Agent documents in .errors_fixes/errors_and_fixes.md   │
│    - User runs `task test` again                            │
│                                                              │
│    If tests pass:                                           │
│    - Agent documents success in .errors_fixes/             │
│      errors_and_fixes.md                                    │
...
#### Command-Based Workflow (Optional)
...
└─────────────────────────────────────────────────────────────┘
                    ↓
┌─────────────────────────────────────────────────────────────┐
│ 4. Agent runs `task test` automatically                     │
└─────────────────────────────────────────────────────────────┘
                    ↓
┌─────────────────────────────────────────────────────────────┐
│ 5. If tests fail:                                           │
│    - Agent parses errors                                    │
│    - Agent checks .errors_fixes/ files                      │
│    - Agent applies fixes (all errors)                       │
│    - Agent documents in .errors_fixes/errors_and_fixes.md   │
│    - Agent runs `task test` again                            │
│    - Repeats until all pass OR max iterations (5)          │
│                                                              │
│    If tests pass:                                           │
│    - Agent reports success                                  │
...
```python
def discover_projects(root_path: Path) -> List[Path]:
    """
    Discover all projects containing .errors_fixes/errors_and_fixes.md
    
    Args:
        root_path: Root directory to search (e.g., /home/user/projects)
    ...
    for path in root_path.rglob('.errors_fixes/errors_and_fixes.md'):
        projects.append(path.parent.parent)  # Go up to project root
    return projects
```
...
def parse_errors_and_fixes(file_path: Path) -> List[ErrorEntry]:
    """
    Parse .errors_fixes/errors_and_fixes.md into structured ErrorEntry objects
    
    Returns:
        List of ErrorEntry objects with:
        - error_signature: str
        - error_type: str
        - file: str
        - line: int
...
def deduplicate_errors(
    ...
    """
    Use AI to find semantically similar errors and merge them.
    
    Strategy:
    1. For each new entry, use LLM to find similar existing entries
    2. Calculate semantic similarity score
    3. If score > threshold: merge into existing entry
    4. If no match: add as new entry
    """
    consolidated = existing_entries.copy()
    
    for new_entry in new_entries:
        best_match = None
        best_score = 0.0
        
        for existing_entry in consolidated:
        ...
        if best_score >= similarity_threshold and best_match:
            ...
            merge_entries(best_match, new_entry)
        else:
            # Add as new entry
...
def calculate_similarity(entry1: ErrorEntry, entry2: ErrorEntry) -> float:
    ...
    prompt = f"""
    Compare these two errors and rate their similarity (0.0-1.0):
    
    Error 1:
    Type: {entry1.error_type}
    Message: {entry1.error_signature}
    Context: {entry1.file}:{entry1.line}
    
    Error 2:
    Type: {entry2.error_type}
    Message: {entry2.error_signature}
    Context: {entry2.file}:{entry2.line}
    
    Consider:
    - Error type (exact match = 1.0, similar = 0.8)
    - Error message similarity (semantic, not exact)
    - Context similarity (same framework/library = 0.7)
    
    Return JSON: {{"similarity": 0.95, "reason": "Both are ConnectionRefusedError in Docker contexts"}}
    """
...
```python
def generate_tags(entry: ErrorEntry) -> List[str]:
    """
    Use AI to generate context tags for error entry
    
    LLM Prompt:
    "Generate 3-5 context tags for this error:
    ...
    Tags should include:
    - Error type (e.g., 'TypeError', 'FileNotFoundError')
    - Framework/library (e.g., 'docker', 'pandas', 'django')
    - Domain (e.g., 'networking', 'file-io', 'database')
    - Platform (e.g., 'windows', 'linux', 'cross-platform')
    
    Return JSON: {{"tags": ["docker", "networking", "connection-refused"]}}"
    """
    prompt = f"""
    Generate 3-5 context tags for this error:
    
    Error: {entry.error_signature}
...
```python
def extract_process_rules(entries: List[ErrorEntry]) -> List[ProcessRule]:
    """
    Extract agent process/workflow rules for coding_tips.md ONLY from entries
    marked as Agent Process Issue (is_process_issue=True). These describe how
    the agent should behave (paths, commands, Docker, etc.), NOT error→fix
    mappings for application code. Error entries (is_process_issue=False) are
    used for fix_repo.md only and must not be passed here.
    
    LLM Prompt:
    "Analyze these agent process issues and extract general rules:
    ...
    {format_process_issues(process_issues)}
    
    For each rule, provide:
    - Rule statement (clear, actionable)
    - Why it's needed (explanation)
    - Examples (✅ good, ❌ bad)
    - Related errors (error types that trigger this rule)
    
    Return JSON: {{"rules": [
        {{
            "title": "Path Handling",
            "rule": "Always use pathlib.Path for file paths",
            "why": "Unix-style paths fail on Windows",
            "examples": {{
                "good": ["Path('data') / 'file.json'"],
                "bad": ["'/data/file.json'"]
            }},
    ...
    response = call_llm(prompt)
    result = json.loads(response)
    return [ProcessRule(**r) for r in result['rules']]
```
...
#### Option 3: Reference in General `rules.mdc` File

**Approach**: If your project has a general `.cursor/rules/global/rules.mdc` or project-specific `.cursor/rules/rules.mdc`, add a reference there.

**Pros**:
- ✅ Centralized location for general project rules
- ✅ Always available if `alwaysApply: true` is used
- ✅ Natural place for project-wide guidelines

**Cons**:
- ⚠️ May cause token bloat if `alwaysApply: true`
- ⚠️ Less specific to Simplified Error Registry

**Implementation**: Add to your general rules file:
```markdown
## Simplified Error Registry

Before coding, check `.errors_fixes/coding_tips.md` for agent process rules:
- Path handling (use `pathlib.Path`)
- Command formats (check taskipy config)
- String formatting best practices
- Docker networking rules

These rules prevent common workflow issues.
```

#### Option 4: User Rules (Global)

**Approach**: Add reference to Cursor User Rules (Settings → Rules) so it applies globally across all projects.
...
**Cons**:
- ⚠️ May cause token bloat if always loaded
- ⚠️ Less project-specific
- ⚠️ Not version-controlled

**Implementation**: Add to User Rules:
```
Before coding, check .errors_fixes/coding_tips.md for agent process rules to prevent common workflow issues.
```

#### Option 5: Explicit Step in Workflow Documentation

**Approach**: Document as a required step in the development workflow, but don't create a rule.

**Pros**:
- ✅ No token usage
- ✅ Flexible - user can reference when needed

**Cons**:
- ❌ Not automatic - relies on user/agent remembering
- ❌ Easy to forget

**Implementation**: Add to project README or workflow documentation:
```markdown
## Before Coding

1. Check `.errors_fixes/coding_tips.md` for agent process rules
2. Review rules for path handling, commands, formatting, etc.
3. Apply rules proactively to prevent errors
```
...
**Critical**: The `errors.mdc` rule is **critical** for the Simplified Error Registry system. It must be carefully crafted with a precise frontmatter description to ensure it's picked up when needed, without using `alwaysApply: true` (which would cause token bloat).
...
The consolidation app discovers projects by scanning for `.errors_fixes/errors_and_fixes.md`:

```python
def discover_projects(root_path: Path) -> List[Path]:
    """Find all projects with .errors_fixes/errors_and_fixes.md"""
    projects = []
    for path in root_path.rglob('.errors_fixes/errors_and_fixes.md'):
        project_root = path.parent.parent  # .errors_fixes -> project root
        projects.append(project_root)
    return projects
```

This ensures:
- Only projects with the system installed are processed
- Missing files don't cause errors
- New projects are automatically discovered **once the folder structure exists**
...
A small Python script (e.g. `consolidation_app/bootstrap.py` or `scripts/bootstrap_errors_fixes.py`) creates the required structure:

```python
# bootstrap_errors_fixes.py
"""Bootstrap .errors_fixes/ in a project so the consolidation app can discover and write to it."""
from pathlib import Path
import argparse

ERRORS_FIXES_HEADER = '''# Errors and Fixes Log
...
def bootstrap(project_root: Path, update_gitignore: bool = True) -> None:
    ef = project_root / ".errors_fixes"
    ef.mkdir(parents=True, exist_ok=True)
    (ef / "errors_and_fixes.md").write_text(ERRORS_FIXES_HEADER, encoding="utf-8")
    (ef / "fix_repo.md").write_text("# Fix Repository\n\n", encoding="utf-8")
    (ef / "coding_tips.md").write_text("# Coding Tips - Agent Process Rules\n\n", encoding="utf-8")
    if update_gitignore:
        gi = project_root / ".gitignore"
        entry = ".errors_fixes/\n"
        if gi.exists():
            text = gi.read_text(encoding="utf-8")
            if ".errors_fixes" not in text:
                gi.write_text(text.rstrip() + "\n" + entry if text.strip() else entry, encoding="utf-8")
        else:
            gi.write_text(entry, encoding="utf-8")

if __name__ == "__main__":
    ap = argparse.ArgumentParser(description="Bootstrap .errors_fixes/ for the consolidation pipeline.")
    ap.add_argument("project_path", type=Path, help="Path to the new project root.")
    ap.add_argument("--no-gitignore", action="store_true", help="Do not add .errors_fixes/ to .gitignore.")
    args = ap.parse_args()
    bootstrap(args.project_path.resolve(), update_gitignore=not args.no_gitignore)
```

Usage:

```bash
python scripts/bootstrap_errors_fixes.py /path/to/new/project
```

Ensure the new project is under `projects_root`; the next consolidation run will discover and process it.

#### C. Config project list + auto-bootstrap in discovery (optional)

For projects outside `projects_root` or for explicit registration, extend `consolidation_config.yaml`:

```yaml
consolidation:
  projects_root: "/home/user/projects"   # rglob scan (existing)
  projects:                              # optional: explicit list (can be outside projects_root)
    - /home/user/other/ProjectX
    - /mnt/workspace/LegacyApp
```
...
def discover_projects(root_path: Path, extra_projects: list[str] | None = None) -> list[Path]:
    ...
    for p in extra_projects or []:
        ...
        if not ef_md.exists():
            bootstrap(project_root, update_gitignore=False)  # re-use bootstrap logic
        projects.append(project_root)
    return list(dict.fromkeys(projects))  # deduplicate
```

To include a new project: add its path to `consolidation.projects`. On the next run, discovery creates `.errors_fixes/errors_and_fixes.md` if missing, then the app reads (empty) and writes `fix_repo.md` and `coding_tips.md` as usual.
...
### Phase 1: Core File Formats (Week 1)
...
**Tasks:**
- [ ] Create `.cursor/rules/global/errors.mdc` template (with hybrid test execution strategy)
  - [ ] **CRITICAL**: Craft precise frontmatter description (do NOT use `alwaysApply: true`)
  - [ ] Description must trigger on: pasted test errors, "There is an error in..." phrases, test failure output
  - [ ] Add "Preventive Checks" section that references `.errors_fixes/coding_tips.md`
  - [ ] Test description with various user error reporting scenarios
  - [ ] Verify rule is triggered when user pastes `task test:*` output
  - [ ] Verify rule is triggered when user says "There is an error in..."
  - [ ] Verify rule is NOT triggered for non-error scenarios (to avoid token bloat)
- [ ] **Create `.cursor/rules/global/coding-tips.mdc` rule** (recommended for proactive prevention):
  - [ ] Craft precise frontmatter description (do NOT use `alwaysApply: true`)
  - [ ] Description must trigger on: "write code", "create file", "use file path", "run command", "Docker" phrases
  - [ ] Reference `.errors_fixes/coding_tips.md` file location
  - [ ] Test that rule triggers during code writing tasks
  - [ ] Verify rule is NOT triggered for non-coding scenarios (to avoid token bloat)
- [ ] **Add test generation requirements** to agent rules:
  - [ ] Instructions to generate tests for all new/modified code
  - [ ] Instructions to advise which test commands to run (`task test:backend`, `task test:frontend`, `task test:integration`)
  - [ ] Test file naming conventions
  - [ ] Test coverage requirements (happy path, edge cases, mocked calls)
- [ ] Create `.cursor/commands/global/test-and-fix.mdc` template (optional global command)
  - [ ] **Note**: This is optional/backup - primary workflow uses `errors.mdc` rule
  - [ ] Command provides alternative auto-fix workflow when user explicitly triggers it
- [ ] **Inspect Taskipy Test Tasks**:
  - [ ] Review all `task test:*` commands in `pyproject.toml` (or equivalent task configuration)
  - [ ] Identify Python scripts that copy test output to clipboard
  - [ ] **Verify test output capture logic**:
    - [ ] Ensure scripts capture the **errors/failures section** (marked by `=== FAILURES ===` or `=== ERRORS ===` in pytest)
    - [ ] Ensure scripts capture the **summary section** (marked by `=== short test summary info ===` in pytest)
    - [ ] Verify scripts do NOT capture irrelevant sections (e.g., test collection output, passing test details)
    - [ ] Test with actual failing tests to confirm correct sections are copied
  - [ ] Update output capture logic if wrong sections are being copied
  - [ ] Document which sections are required for agent error identification
- [ ] Test agent workflow end-to-end (both manual and command-based modes)
- [ ] **Verification: Test Generation**
  - [ ] Create test scenario: Ask agent to create new code file
  - [ ] Verify agent automatically generates corresponding test file
  - [ ] Verify test file follows naming conventions
  - [ ] Verify test file includes happy path, edge cases, and mocked calls
  - [ ] Verify agent advises which test command to run (`task test:backend`, `task test:frontend`, `task test:integration`)
- [ ] **Verification: Test Command Advice**
  - [ ] Create backend code → Verify agent advises `task test:backend`
  - [ ] Create frontend code → Verify agent advises `task test:frontend`
  - [ ] Create integration code → Verify agent advises `task test:integration`
  - [ ] Create mixed changes → Verify agent advises all relevant test commands
- [ ] **Verification: Test Output Capture**
  - [ ] Run `task test:backend` with failing tests → Verify clipboard contains errors/failures section and summary
  - [ ] Run `task test:frontend` with failing tests → Verify clipboard contains errors/failures section and summary
  - [ ] Run `task test:integration` with failing tests → Verify clipboard contains errors/failures section and summary
  - [ ] Verify agent can properly parse errors from captured output
- [ ] Document agent instructions
- [ ] Test safety limits (max iterations, retry limits)

**Deliverables:**
- Agent rules file (with test generation requirements)
- Optional command file
- Documentation
- Test results (both modes)
- Verification test cases and results
- Updated test output capture scripts (if needed)
- Documentation of required test output sections

### Phase 2.5: Test Output Capture Verification (Week 2)

**Critical Note**: Test output capture must include the correct sections for agent error identification.

**Required Test Output Sections**:

1. **Errors/Failures Section**:
   - Marked by `=== FAILURES ===` or `=== ERRORS ===` in pytest output
   - Contains detailed traceback and error messages for each failing test
   - Required for agent to identify specific errors

2. **Summary Section**:
   - Marked by `=== short test summary info ===` in pytest output
   - Contains concise summary of all failures
   - Required for agent to get overview of all errors

**Common Issues**:
- Scripts may capture entire test output (including passing tests, collection info)
- Scripts may miss the errors/failures section if output format differs
- Scripts may miss the summary section if it appears at the end

**Verification Steps**:
1. Run each `task test:*` command with intentionally failing tests
2. Check clipboard contents after test run
3. Verify clipboard contains:
   - `=== FAILURES ===` or `=== ERRORS ===` section
   - `=== short test summary info ===` section
4. Verify clipboard does NOT contain:
   - Excessive passing test output
   - Test collection information (unless relevant)
   - Irrelevant sections

**Files to Inspect**:
- `scripts/test_backend.py` (or equivalent)
- `scripts/test_frontend.py` (or equivalent)
- `scripts/test_integration.py` (or equivalent)
- Any other test scripts referenced in Taskipy configuration
- `tools/error_parser.py` (for parsing logic reference)

**Note**: The Python code associated with Taskipy test tasks automatically copies a portion of the test results output written to the terminal. However, it sometimes copies the wrong section. Tests produce specific **errors/failures sections** and **summary sections**, which is what the agent requires to identify errors properly. Ensure the capture logic targets these specific sections.
...
**Tasks:**
- [ ] Discovery module (scan for `.errors_fixes/errors_and_fixes.md`)
- [ ] Parser module
- [ ] Basic deduplication (exact match first)
...
### Phase 5: Docker & Scheduling (Week 4)
...
- [ ] Documentation
- [ ] Portability testing (copy folder to new project)
...
See individual module sections for detailed prompts. Common patterns:

1. **Similarity Calculation**: Compare two errors, return similarity score
2. **Tag Generation**: Analyze error, return context tags
3. **Rule Extraction**: Analyze process issues, return general rules
4. **Fix Explanation**: Explain why a fix works
...
consolidation:
  ...
  min_success_count: 1
  errors_fixes_folder: ".errors_fixes"  # Folder name
...
output:
  fix_repo_filename: "fix_repo.md"
  coding_tips_filename: "coding_tips.md"
  clear_errors_and_fixes: true  # Clear contents after consolidation
```
...
**Note**: The folder is git-ignored because:
- Session logs are ephemeral (cleared daily)
- Consolidated files are regenerated by consolidation app
- Keeps repository clean
- Each developer maintains their own session logs
...
- Updated to use `.errors_fixes/` folder structure
- Added portability section
- Updated to use global Cursor rules/commands (`.cursor/rules/global/errors.mdc` and `.cursor/commands/global/test-and-fix.mdc`)
- **CRITICAL**: Emphasized careful crafting of `errors.mdc` frontmatter description (no `alwaysApply: true` to avoid token bloat)
- Added consolidation cleanup (clear contents, not delete file)
- Updated all file paths to use `.errors_fixes/` prefix
- Added git-ignore instructions
- Clarified that `test-and-fix.mdc` command is optional/backup, primary workflow uses `errors.mdc` rule